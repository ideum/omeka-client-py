...
class JsonToCsv:
	...
	def process_json(self, json_input):
		...
		for obj in json_input:
			for key in columns:
				if key.find(".") >= 0:
					output += str(self.nested_item(key.split("."), obj))
				elif key.find("[") >= 0:
					key, index = self.keypath_index(key)
					output += str(obj[key][index])
				else:
					output += str(obj[key])
				output += ","
	...
	def nested_item(self, keypath, json_input):
		if (len(keypath) == 1):
			...
			return self.nested_item(keypath[1:len(keypath)], json_input[key][index])
		else:
			return self.nested_item(keypath[1:len(keypath)], json_input[keypath[0]])
